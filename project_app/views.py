...
def register(request):
    if request.method == 'POST':
        ...
        # Domain whitelist: allow only the exact provider domains (no other custom domains)
        ...
        if domain not in allowed_domains:
            return render(request, 'authentication/register.html',
                        {'register_check_key': "Email domain is not allowed. Please use one of the accepted providers (e.g. gmail.com, yahoo.com, outlook.com, etc.)."})
...
def landing(request):
    ...
    # Sum of incomes and expenses — total money flowing through the system
    ...
    # active users: users who have at least one income or expense
    active_users = Registration.objects.filter(
        Q(income__isnull=False) | Q(expense__isnull=False)
    ).distinct().count()

    # total transactions: both income and expense entries
    income_transactions = Income.objects.count()
    expense_transactions = Expense.objects.count()
    total_transactions = income_transactions + expense_transactions

    # derive a simple satisfaction / engagement rate as % of active users over total registrations
    if total_registered > 0:
        satisfaction_rate = round((active_users / total_registered) * 100)
    else:
        satisfaction_rate = 0

    context = {
        # Active users (users who actually performed transactions)
        'user_count': int(active_users),
        # Total registered users (for badges / CTA)
        'actual_user_count': int(total_registered),
        # Money managed across all users and transactions
        'money_managed': int(money_managed),
        # Total transactions (income + expense)
        'total_transactions': int(total_transactions),
        # Engagement / satisfaction proxy
...
def analytics(request):
    ...
    # Build 12-element arrays (Jan..Dec) with totals for the current year
    monthly_income_by_month = [0.0] * 12
    monthly_expense_by_month = [0.0] * 12
    try:
        for item in monthly_income:
            m = item['month'].month
            monthly_income_by_month[m - 1] = float(item['total'] or 0)
    except Exception:
    ...
    try:
        for item in monthly_expense:
            m = item['month'].month
            monthly_expense_by_month[m - 1] = float(item['total'] or 0)
    except Exception:
        pass
    ...
    context = {
        ...
        # Monthly arrays for charts (Jan..Dec)
        'monthly_income_by_month_json': json.dumps(monthly_income_by_month),
        'monthly_expense_by_month_json': json.dumps(monthly_expense_by_month),
    }
